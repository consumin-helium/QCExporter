...
class QC_Properties(PropertyGroup):
    ...
    exportcollection : PointerProperty(
    ...
    ## panel in the vmt panel
    customimagedirectory: StringProperty(
        name="custom? Materials", description="The path of the model's materials relative to the materials/ dir.",
        default="models/")


    exportpath: StringProperty(
        name="Export Folder", description="The sub path of the export directory. Default current path",
        default="//")
    sequence: StringProperty(
        name="Idle Sequence", description="The name if the sequence file, default idle.smd",
        default="idle.smd")
    ...
    staticprop: BoolProperty(
        name="Static Prop", default=True)
    opaque: BoolProperty(
        name="Opaque Prop", default=True)
    activated: BoolProperty(
        name="Has Activity Sequence", default=False)
    scale: IntProperty(
        name="Scale",
        default=1
    )
    ...
    activity: StringProperty(
        name="Activity Event", description="Event that starts the anim",
        default="ACT_VM_IDLE")
    ...
    def execute(self, context):
        ...
        if qcgen.save_qc_file:
            if qc_text.filepath:
                ...
                qc_path = os.path.join(os.path.dirname(bpy.data.filepath) + "/Export/", qc_path) #old
            else:
                qc_path = os.path.splitext(os.path.basename(bpy.data.filepath))[0] + ".qc"
                qc_path = os.path.join(os.path.dirname(bpy.data.filepath) + "/Export/", qc_path) #old
            ...
            self.report({'INFO'}, "Saved file " + qc_path)
...
class QC_OT_AutofillVS(Operator):
    ...
    def execute(self, context):
        ...
        gamedir = "D:/Steam/steamapps/common/Counter-Strike Global Offensive/"
...
class QC_PT_QCPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(qcgen, "exportcollection")
        ...
        # params box
        #box = layout.box()
        #box.prop(qcgen, "sequence")
        #box.prop(qcgen, 'fps')
        #box.prop(qcgen, 'activity')

        split = layout.split()
        col = split.column()
        col.prop(qcgen, "staticprop")
        col.prop(qcgen, "opaque")
        col.prop(qcgen, "activated")
